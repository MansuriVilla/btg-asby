document.addEventListener("DOMContentLoaded", function() {
    const HUBBOX_VERSION = '3.3.6';

    if (!window.HubBox) window.HubBox = {};
    /* Config */
    if (!window.HubBox.theme) window.HubBox.theme = 'ups';
    if (!window.HubBox.configId) window.HubBox.configId = 'blueteesgolf';
    ...
    if (!window.HubBox.D2RField) window.HubBox.D2RField = 'address2'; // Accepted values: address1, address2, company. Default: address1
    ...
    (function(){
        ...
        var detectShouldLockAddress = function() {
            var regexCheck = /D2R|Pickup|HubBox|S2S|HBC|HBCF|PCP|PCPF|/;
            ...
            switch(window.HubBox.theme.toLowerCase()) {
                case 'dpd':
                    ...
                    if (hasCompanyEnabled) { //Company field exists
                        ...
                        document.querySelectorAll('[name="'+i.address1name+'"]').forEach((elm) => {
                            if (cp.meta.type && cp.meta.type == "private"){
                                //Private CPs shouldn't prefix S2S
                ...
                case 'ups':
                    if (window.HubBox.isD2REnabled) {
                        var prefix = ".D2R." + cp.id + ".";
                        var postfix = "";
                        var ping = JSON.parse(sessionStorage.hubBox_ping).guid; 
                    } else {
                        var prefix = "";
                        var postfix = " Pickup";
                    }
        ...
        window.HubBox.lockFields = function() {
            ...
            window.HubBox.checkoutEnhancementWatch = setInterval(function () {
                if (!document.querySelector('div.hubbox-checkout-confirmation-container')) {
                    ...
                    var hide = [
                        // crafty clicks https://craftyclicks.co.uk/
                        ".cc_address_search",
            ...
            !this.settings.storeConfig.hasOwnProperty("no_company") || 
              ...
              this.jQ(i.province) && this.jQ(i.province).val(getUSStateCode(t.address.county)),
                ...
                if (!this.settings.storeConfig.hasOwnProperty("no_company") || 
                  ...
                  1 !== this.settings.storeConfig.no_company
                  ){ //Choose the field to add D2R prefix -- default fallback: address2
                    if (window.HubBox.D2RField && window.HubBox.D2RField.toLowerCase() == "address2") {
                        (this.jQ(i.company).val(t.name),
                        this.jQ(i.address2).val(e),
                        t.address.street2 && 0 < t.address.street2.trim().length ? this.jQ(i.address2).val(t.address.street2) : this.jQ(i.address2).val(e)) 
                    } else if (window.HubBox.D2RField && window.HubBox.D2RField.toLowerCase() == "company") {
                    ...
                    } else if (window.HubBox.D2RField && window.HubBox.D2RField.toLowerCase() == "address1") {
                        (this.jQ(i.address2).val(t.name),
                        this.jQ(i.address1).val(e),
                        t.address.street1 && 0 < t.address.street1.trim().length ? this.jQ(i.address2).val(t.address.street1) : this.jQ(i.address2).val("-")) 
                    } else {
                        //default: address2
                        (this.jQ(i.address1).val(t.name),
                        this.jQ(i.address2).val(e),
                        t.address.street2 && 0 < t.address.street2.trim().length ? this.jQ(i.address2).val(t.address.street2) : this.jQ(i.address2).val("-")) 
                    }
                    ...
                    this.jQ(i.province) && this.jQ(i.province).val(getUSStateCode(t.address.county)),
                    this.jQ(i.postCode).val(t.address.postcode),
                    this.jQ(i.country).val(getCountryString(t.address.country))//todo fix country code selection 
                }
            } else {
                //HubBox
                if (!this.settings.storeConfig.hasOwnProperty("no_company") || 
