...
if (console && console.log) {
  console.log(
    "Blueteesgolf theme (" +
      ...
      ") by blueteesgolf | Learn more at blueteesgolf.com",
  );
}
...
(function () {
  ...
  theme.a11y = {
    trapFocus: function (options) {
      ...
      function getVisibleFocusable(container) {
        var focusableEls = container.querySelectorAll(
          'button:not([disabled]), [href]:not([disabled]), input:not([disabled]):not([type="hidden"]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex^="-"]):not([disabled])',
        );
  ...
  theme.CartForm = (function () {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function (html) {
        ...
        var len = $(".ctm_upsell_product_list > [data-main-id]").length;
        var count = 0;
        $(".ctm_upsell_product_list > [data-main-id]").each(function (e) {
          var _len = 0;
          var _speaker = 0;
          var upsell_id = parseInt($(this).attr("data-prod-id"));
          $(items)
            .find(".cart__item")
            .each(function (e) {
        ...
        if (len == count) {
          $(".ctm_upsell_product").hide();
        } else {
          $(".ctm_upsell_product").show();
        }

        var text = $(".discount-amout").data("text");
        var limit =
          parseInt($(".discount-amout").data("limit")) * Shopify.currency.rate;
        var success = $(".discount-amout").data("success-text");
        var final_val = theme.Currency.formatMoney(
          limit - subtotal,
          theme.settings.moneyFormat,
        );

        var compare_val = parseInt(subtotal);
        var percentage = parseFloat((compare_val / limit) * 100);

        if (compare_val < limit) {
          $(".ctm_offer_main_wrapper .offers").show();
          $(".ctm_offer_main_wrapper .ctm_success_message").hide();
          $(".ctm_offer_main_wrapper .offers").html(
            text.replace("[amount]", final_val),
          );
          $(".bg_progress_bar .progress").css("width", percentage + "%");
          Cookies.set("offer_valid", false, { path: "/" });
        } else {
          $(".ctm_offer_main_wrapper .offers").hide();
          $(".ctm_offer_main_wrapper .ctm_success_message").show();
          $(".bg_progress_bar .progress").css("width", "100%");
          Cookies.set("offer_valid", true, { path: "/" });
        }
        console.log(subtotal, count);
        ...
        if (theme.lastFocusedQtyBtn) {
          var restoredInput = document.getElementById(
            theme.lastFocusedQtyBtn.id,
          );
          if (restoredInput) {
            var btn = restoredInput.parentElement.querySelector(
              ".js-qty__adjust--" + theme.lastFocusedQtyBtn.action,
            );
            if (btn) btn.focus();
          }
      ...
      quantityChanged: function (evt) {
        ...
        if (activeBtn && activeBtn.classList.contains("js-qty__adjust")) {
          ...
          if (inputEl) {
            theme.lastFocusedQtyBtn = {
              id: inputEl.id,
              action: activeBtn.classList.contains("js-qty__adjust--plus")
                ? "plus"
                : "minus",
            };
          }
        ...
        var existingItemEl = this.products.querySelector(
          '[data-key="' + key + '"]',
        );
        var itemTitle = "Item";
        if (existingItemEl) {
        ...
        theme.cart
          ...
          .then(
            function (cart) {
              ...
              var formattedSubtotal = theme.Currency.formatMoney(
                cart.total_price,
                theme.settings.moneyFormat,
              ).replace(/<\/?[^>]+(>|$)/g, "");
              var announcement = "";
              ...
              if (updatedItem) {
                ...
                if (theme.lastFocusedQtyBtn && theme.lastFocusedQtyBtn.action) {
                  actionText =
                    theme.lastFocusedQtyBtn.action === "plus"
                      ? "increased"
                      : "decreased";
                }
                announcement =
                  (updatedItem.product_title || itemTitle) +
                  " quantity " +
                  actionText +
                  " to " +
                ...
                announcement =
                  itemTitle +
                  " removed from cart. Subtotal is now " +
                  formattedSubtotal;
              } else {
                announcement =
                  "Cart updated. Subtotal is now " + formattedSubtotal;
              }
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      ...
      open: function (evt, returnFocusEl) {
        ...
        theme.a11y.trapFocus({
          ...
          elementToFocus:
            this.drawer.querySelector(".drawer__title") || this.drawer,
          ...
          .then(
            function (data) {
              if (data.status === 422) {
                ...
                if (
                  data.id == "40475982921925" ||
                  data.id == "40475982954693" ||
                  data.id == "41764974133445" ||
                  data.id == "40475982987461"
                ) {
                  var formParams = {
                    id: 41354207199429,
                  };
                ...
                if ($("#free-ringer-premium-product").is(":checked")) {
                  var formParams = {
                    id: 42376729329861,
                ...
                // ORIGINAL SOFT CASE SCRIPT

                // if($('#playerplus-soft-case-upshell').is(':checked'))
                // {
                //     var formParams = {
                //       'id':42924529811653,
                //       'quantity': 1,
                ...
                if ($("#playerplus-player-go-upshell").is(":checked")) {
                  var formParams = {
                    // BLACK 'id':42817543536837,
                    id: 42817543569605,
                    quantity: 1,
                  };
                ...
                if ($("#playmaker-hard-new-case").is(":checked")) {
                  var formParams = {
                    // BLACK 'id':42817543536837,
                    id: 45859460120773,
                    quantity: 1,
                ...
                /*
            if ($('#free-premium-product').is(':checked') && $('#hard-shell-case').is(':checked')) {
              // Add first product
              $.ajax({
                  url: "/cart/add",
                  type: "post",
                  ...
                  success: function () {
                      ...
                      // Add second product after the first one succeeds
                      $.ajax({
                          url: "/cart/add",
                          type: "post",
                          data: {
              ...
              if ($('#free-premium-product').is(':checked')) {

                  $.getJSON('/cart.js', function (cart) {
                    // Now, check if the membership product is already in the cart
                    var itemExists = cart.items.some(item => item.id === 44528451059909);
                ...
                if (
                  $("#6month-free-premium-product").is(":checked") &&
                  $("#hard-shell-case").is(":checked")
                ) {
                  // Add first product

                  $.getJSON("/cart.js", function (cart) {
                    // Now, check if the membership product is already in the cart
                    var itemExists = cart.items.some(
                      (item) => item.id === 46056776990917,
                    );
                  ...
                  if ($("#6month-free-premium-product").is(":checked")) {
                    $.getJSON("/cart.js", function (cart) {
                      // Now, check if the membership product is already in the cart
                      var itemExists = cart.items.some(
                        (item) => item.id === 46056776990917,
                  ...
                  if ($("#hard-shell-case").is(":checked")) {
                    $.ajax({
                      ...
                      success: function () {
                        document.dispatchEvent(
                          new CustomEvent("ajaxProduct:added", { detail: {} }),
                        );
                      },
                    });
                  }
                ...
                /* custom code for Player+ GPS Speaker */
                // if (window.location.href.indexOf("player-gps-speaker") > -1 || window.location.href.indexOf("ringerpack") > -1 || window.location.href.indexOf("player-pro") > -1 || window.location.href.indexOf("captain-pro") > -1 || window.location.href.indexOf("captain-air") > -1) {
                if (
                  window.location.href.indexOf("player-gps-speaker") > -1 ||
                  window.location.href.indexOf("ringerpack") > -1
                ) {
                  if ($("#free-premium-product1").is(":checked")) {
                    /*var formParams = {
                      'id':42496814088389,
                      'selling_plan':2953052357
                    };
                    var formParams = {
                      'id':42279859028165,
                      'selling_plan':2791211205
                ...
                if (
                  document.querySelectorAll(
                    'input[name="offer-checkbox"]:checked',
                  ).length === 0
                ) {
                  $("#offer-popup").show();
                  $("#popup-preorder").click(function () {
                    function removeItemPlayer() {
                      $.ajax({
                        type: "POST",
                        url: "/cart/change.js",
                        dataType: "json",
                    ...
                    if (
                      $("#popup-free-premium-product-2-year").is(":checked")
                    ) {
                      var formParams = {
                        id: 42496814088389,
                        selling_plan: 2953052357,
                ...
                /* custom code for RINGER GPS HANDHELD  */

                if (window.location.href.indexOf("ringer") > -1) {
                  if ($("#free-ringer-premium-product1").is(":checked")) {
                    /*var formParams = {
                      'id':42496818708677,
                      'selling_plan':2953019589
                    };*/
                    var formParams = {
                      id: 42279859028165,
                      selling_plan: 2791211205,
                ...
                /*
            if($('#free-premium-product-3-year-ringer').is(':checked'))
            {    
                var formParams = {
                  'id':42387490439365,
                  'selling_plan':2866184389
                };
            ...
            */
                ...
                if (
                  document.querySelectorAll(
                    'input[name="offer-checkbox-ringer"]:checked',
                  ).length === 0
                ) {
                  $("#offer-popup-ringer").show();

                  $("#popup-preorder").click(function () {
                    function removeItemRinger() {
                      $.ajax({
                        type: "POST",
                        url: "/cart/change.js",
                        dataType: "json",
                    ...
                    if (
                      $("#popup-free-premium-product-2-year-ringer").is(
                        ":checked",
                      )
                    ) {
                      var formParams = {
                    ...
                    /*if($('#popup-free-premium-product-ringer-popup').is(':checked'))
                      {    
                          var formParams = {
                            'id':42376729329861,
                            'selling_plan':2866151621
                      ...
                      if($('#popup-free-premium-product-2-year-ringer').is(':checked'))
                          {    
                              var formParams = {
                                'id':42496818708677,
                                'selling_plan':2953019589
                      ...
                      if($('#popup-free-premium-product-3-year-ringer-popup').is(':checked'))
                      {    
                          var formParams = {
                            'id':42387490439365,
                            'selling_plan':2866184389
                ...
                /* custom code for Player pack go */
                if (
                  $("#save-premium-member-product-player-pack-go").is(
                    ":checked",
                  )
                ) {
                  var formParams = {
                    id: 43469941571781,
                    selling_plan: 3518890181,
                  };
                ...
                if (
                  document.querySelectorAll(
                    'input[name="save-checkbox-player-pack-go"]:checked',
                  ).length === 0
                ) {
                  $("#save-popup-player-pack-go").show();

                  $("#popup-preorder").click(function () {
                    if (
                      $("#popup-save-premium-member-product-player-pack-go").is(
                        ":checked",
                      )
                    ) {
                      var formParams = {
                      ...
                      $.ajax({
                        ...
                        data: formParams,
                        success: function () {
                          $("#save-popup-player-pack-go").hide();
                          document.dispatchEvent(
                            new CustomEvent("ajaxProduct:added", {
                              detail: {},
                            }),
                ...
                /* custom code for S3, S2 and Player   */
                if (
                  $("#free-premium-member-product-rangefinder-three").is(
                    ":checked",
                  )
                ) {
                  var formParams = {
                    id: 42436038262981,
                    selling_plan: 2919858373,
                  };
                ...
                /*if (document.querySelectorAll('input[name="offer-checkbox-rangefinder-three"]:checked').length === 0)
            {
                $('#offer-popup--rangefinder-three').show();

                $("#popup-preorder").click(function(){
                      if($('#popup-free-premium-member-product-rangefinder-three').is(':checked'))
                      {    
                          var formParams = {
                            'id':42436038262981,
                            'selling_plan':2919858373
                           ...
                           $.ajax({
                              ...
                              data: formParams,
                              success: function(){ 
                                $('#offer-popup--rangefinder-three').hide();
                                document.dispatchEvent(new CustomEvent('ajaxProduct:added', {
                                  detail: {}
                                }));
                              },
                ...
                /* custom code for series 3max plus and Player go */
                if ($("#6month-membership-save-plan").is(":checked")) {
                  var formParams = {
                    id: 42945507885253,
                  };
                ...
                if ($("#6month-membership-free").is(":checked")) {
                  /*var formParams = {
                  'id':44384307118277
                };*/
                ...
                /*
            if ($('#3month-membership-free').is(':checked')) {
              // First, retrieve the cart data
              $.getJSON('/cart.js', function (cart) {
                  // Now, check if the membership product is already in the cart
                  var itemExists = cart.items.some(item => item.id === 42279859028165);
            ...
            */
                ...
                if ($("#1month-membership-free").is(":checked")) {
                  // First, retrieve the cart data
                  $.getJSON("/cart.js", function (cart) {
                    // Now, check if the membership product is already in the cart
                    var itemExists = cart.items.some(
                      (item) => item.id === 46056795209925,
                    );
                ...
                if (
                  document.querySelectorAll(
                    'input[name="new-offer-premium-save"]:checked',
                  ).length === 0
                ) {
                  $("#offer-popup-series3max-plus-player-go").show();

                  $("#popup-preorder").click(function () {
                    if (
                      $("#popup-free-series3max-plus-player-go").is(":checked")
                    ) {
                      var formParams = {
                        id: 42436038262981,
                        selling_plan: 2919858373,
                      ...
                      $.ajax({
                        ...
                        data: formParams,
                        success: function () {
                          $("#offer-popup-series3max-plus-player-go").hide();
                          document.dispatchEvent(
                            new CustomEvent("ajaxProduct:added", {
                              detail: {},
                            }),
                    ...
                    if ($("#popup-premium-6month-save").is(":checked")) {
                      var formParams = {
                        id: 42945507885253,
                      };
                    ...
                    if ($("#popup-premium-6month-free").is(":checked")) {
                      var formParams = {
                        id: 42945507918021,
                      };
                ...
                if ($("#rainmakerPreOrderUpsell").is(":checked")) {
                  /*
                // Get the selected variant id from the swatches
                var selectedVariantId = $('.quick-add-swatches input[type="radio"]:checked').val();

                // Safety fallback (in case nothing is checked)
                if (!selectedVariantId) {
                  selectedVariantId = $('.quick-add-swatches input[type="radio"]').first().val();
                }
  ...
  theme.QtySelector = (function () {
    ...
    /*  
      |--------------------------------------------------------------------------
      |   MAP QUALIFYING VARIANT ➜ FREE VARIANT
      |   (When qualifying variant hits qty 0 → remove free variant)
      |--------------------------------------------------------------------------
      */
    ...
    var FREE_GIFTS_BY_VARIANT = {
      ...
      44119278584005: "42279859028165", // 42279859028165 one year id

      45724258697413: "45793869299909",
      45724258468037: "46056776990917", // 46056776990917 six month id
      45724250669253: "46056776990917", // 46056776990917 six month id
      45724259942597: "46056776990917", // 46056776990917 six month id
      42373188485317: "46056776990917", // 46056776990917 six month id

      42817543536837: "46056795209925", // 46056776990917 six month id
      44599654088901: "46056795209925", // 46056776990917 six month id
      42373188485317: "46056795209925", // 46056776990917 six month id
      46039346413765: "46056795209925", // 46056795209925 one month id
      41354208280773: "46056795209925", // 46056795209925 one month id
      41354207166661: "46056795209925", // 46056795209925 one month id
      42924526043333: "46056795209925", // 46056795209925 one month id
      42924529811653: "46056795209925", // 46056795209925 one month id
      45110734618821: "46056795209925", // 46056795209925 one month id
      42935689937093: "46056795209925", // 46056795209925 one month id
      45859460120773: "46056795209925", // 46056795209925 one month id
    };

    /*  
      |--------------------------------------------------------------------------
      |   REMOVE FREE GIFT HELPERS
      |--------------------------------------------------------------------------
      */
      ...
      return fetch("/cart.js?_=" + Date.now(), {
        ...
        .then((cart) => {
          ...
          var hasRemainingQualifyingItem = cart.items.some(function (item) {
            ...
            if (String(giftIdForThisItem) === String(freeVariantId))
              return true;
    ...
    function QtySelector(el, options) {
      ...
      var minAttr = this.input.getAttribute("min");
      this.minValue = minAttr !== null ? parseInt(minAttr, 10) : 1;
      if (isNaN(this.minValue)) this.minValue = 1;

      // Get variant id from parent .cart__item
      this.cartItem = this.wrapper.closest(".cart__item");
      this.lineKey = this.cartItem
        ? this.cartItem.getAttribute("data-key")
        : this.input.dataset.id;

      this.variantId = this.lineKey
        ? this.lineKey.split(":")[0] // variant ID before ":"
        : null;

      var defaults = {
        namespace: null,
        isCart: false,
        key: this.lineKey || this.input.dataset.id,
      };
    ...
    QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
      ...
      _getQty: function () {
        var qty = parseInt(this.input.value, 10);
      ...
      _change: function (qty) {
        ...
        // CART logic allows qty = 0 to remove item
        if (this.options.isCart) {
          if (qty < 0) qty = 0;
        } else {
          // PDP/default: enforce minValue
        ...
        if (this.options.isCart) {
          ...
          var hasFreeGift =
            this.variantId &&
            FREE_GIFTS_BY_VARIANT.hasOwnProperty(String(this.variantId));

          // 1. Update/remove MAIN CART LINE via Shopify AJAX
          fetch("/cart/change.js", {
            ...
            .then((cart) => {
              // A11y announcement
              var updatedItem = cart.items.find(
                (item) =>
                  item.key === lineKey ||
                  item.id == lineKey ||
                  item.variant_id == lineKey,
              );
              var actionText = "updated";
              if (theme.lastFocusedQtyBtn && theme.lastFocusedQtyBtn.action) {
                actionText =
              ...
              if (updatedItem) {
                announcement =
                  (updatedItem.product_title || productTitle) +
                  " quantity " +
                  actionText +
                  " to " +
                  updatedItem.quantity +
            ...
            .then(() => {
              // 3. Refresh mini cart drawer
              document.dispatchEvent(
                new CustomEvent("ajaxProduct:added", { detail: {} }),
              );
            })
            .catch((err) =>
              console.error("Error updating main product quantity:", err),
            );

          return; // stop here
        }

        // NON-cart legacy event support
        document.dispatchEvent(
  ...
  theme.Slideshow = (function () {
    ...
    slideshow.prototype = Object.assign({}, slideshow.prototype, {
      ...
      initChildNav: function () {
        ...
        this.childNavLinks[this.args.initialIndex].setAttribute(
          "aria-current",
          "true",
        );
  ...
  DOMready(function () {
    ...
    // OLD SCRIPT FOR CART YOU MAY LIKE SECTION WITHOUT ADDING CLASS
    // $(document).on('click','.ctm_upsell_product_list .bottom-add-to-cart',function(e) {
    //    e.preventDefault();
    ...
    $(document).on(
      "click",
      ".ctm_upsell_product_list .bottom-add-to-cart",
      function (e) {
        e.preventDefault();
        var $btn = $(this);
    ...
    $(document).on("click", "#max-spekar-both-add", function (e) {
      e.preventDefault();
      ...
      $.ajax({
        ...
        success: function (data) {
          //alert('1');
          $(".button.new-button .btn").removeClass("btn--loading");
          document.dispatchEvent(
            new CustomEvent("ajaxProduct:added", {
              detail: {},
            }),
    ...
    $(document).on("click", "#max-ringer-both-add", function (e) {
      e.preventDefault();
      $(this).addClass("btn--loading");
      var formParams = {
        items: [
...
/*
 $(document).on('click', '.product-atc_cta a.btn', function (e) {
        e.preventDefault();
        $(this).addClass('btn--loading');
        ...
        $.getJSON('/cart.js', function(cart) {
            ...
            var existingItem = cart.items.find(item => item.id === 42266534052037);
            ...
            if (existingItem) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('.product-atc_cta a.btn').removeClass('btn--loading');
                        document.dispatchEvent(new CustomEvent('ajaxProduct:added', { detail: {} }));
                    },
                    error: function(err) {
                        console.log(err);
...
$(document).on("click", "#watch-hard-shell-carrying-case", function (e) {
  e.preventDefault();
  var $btn = $(this);
  $btn.addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    var itemExists = cart.items.some(
      (item) => item.id === 42279859028165 || item.id === 44528451059909,
    );

    var formParams;

    if (itemExists) {
      // If the restricted item is in the cart, only increase the quantity of 44594032378053
      //var existingItem = cart.items.find(item => item.id === 44594032378053);
...
$(document).on("click", "#bottom-only-watch", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  var formParams = {
    id: 44599654088901,
  ...
  $.ajax({
    ...
    success: function (data) {
      //alert('1');
      $("#bottom-only-watch").removeClass("btn--loading");
      document.dispatchEvent(
        new CustomEvent("ajaxProduct:added", {
          detail: {},
        }),
...
$(document).on("click", ".popup-modal-bottom a.btn", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    // Check if items are already in the cart
    var existingItem = cart.items.find((item) => item.id === 42279859028165);
    ...
    if (existingItem) {
      ...
      $.ajax({
        ...
        contentType: "application/json",
        success: function (data) {
          $(".popup-modal-bottom a.btn").removeClass("btn--loading");
          $("#learnMoreModal").hide();
          document.dispatchEvent(
            new CustomEvent("ajaxProduct:added", { detail: {} }),
          );
...
$(document).on("click", ".popup-modal-bottom #1moUpshellFree", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    // Check if items are already in the cart
    var existingItem = cart.items.find((item) => item.id === 44289635451077);
    ...
    if (existingItem) {
      ...
      $.ajax({
        ...
        contentType: "application/json",
        success: function (data) {
          $(".popup-modal-bottom #1moUpshellFree").removeClass("btn--loading");
          $("#accessoriesPopUp").hide();
          document.dispatchEvent(
            new CustomEvent("ajaxProduct:added", { detail: {} }),
          );
        },
...
$(document).on("click", ".popup-modal-bottom #3moUpshellFree", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    // Check if items are already in the cart
    var existingItem = cart.items.find((item) => item.id === 44384307118277);
    ...
    if (existingItem) {
      ...
      $.ajax({
        ...
        contentType: "application/json",
        success: function (data) {
          $(".popup-modal-bottom #3moUpshellFree").removeClass("btn--loading");
          $("#rangfinderPopUp").hide();
          document.dispatchEvent(
            new CustomEvent("ajaxProduct:added", { detail: {} }),
          );
        },
...
$(document).on(
  "click",
  ".popup-modal-bottom #1month-membership-free-popup",
  function (e) {
    e.preventDefault();
    $(this).addClass("btn--loading");
    ...
    $.getJSON("/cart.js", function (cart) {
      // Check if items are already in the cart
      var existingItem = cart.items.find((item) => item.id === 46056795209925);

      if (existingItem) {
        // If item exists, update quantity

        alert("Membership product is already in the cart.");
        $(".popup-modal-bottom .upshell-popup-button").removeClass(
          "btn--loading",
        );
        $("#accessoriesPopUp").hide();
        $("#one_month_popup").hide();
        ...
        /*$.ajax({
                    ...
                    contentType: 'application/json',
                    success: function(data) {
                        $('.popup-modal-bottom .upshell-popup-button ').removeClass('btn--loading');
                        $('#accessoriesPopUp').hide();
                        $('#one_month_popup').hide();
                        document.dispatchEvent(new CustomEvent('ajaxProduct:added', { detail: {} }));
                    },
...
$(document).on(
  "click",
  ".popup-modal-bottom #rainmakerPreOrderUpsell--popup",
  function (e) {
    e.preventDefault();
    $(this).addClass("btn--loading");
    ...
    $.getJSON("/cart.js", function (cart) {
      // Check if items are already in the cart
      var existingItem = cart.items.find((item) => item.id === 45793869299909);
...
$(document).on(
  "click",
  ".popup-modal-bottom #6month-free-premium-product-popup",
  function (e) {
    e.preventDefault();
    $(this).addClass("btn--loading");
    ...
    $.getJSON("/cart.js", function (cart) {
      // Check if items are already in the cart
      var existingItem = cart.items.find((item) => item.id === 46056776990917);
      ...
      if (existingItem) {
        ...
        $("#playerplus").hide();
        $("#playermakerplus").hide();
        $("#six_month_popup").hide();
...
$(document).on(
  "click",
  ".popup-modal-bottom #6month-membership-free",
  function (e) {
    e.preventDefault();
    $(this).addClass("btn--loading");
    ...
    $.getJSON("/cart.js", function (cart) {
      // Check if items are already in the cart
      var existingItem = cart.items.find((item) => item.id === 44371793477829);
...
$(document).on("click", ".tow_content-cta .claim-now-btn", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    ...
    if (existingItem) {
      ...
      $.ajax({
        ...
        contentType: "application/json",
        success: function (data) {
          $(".tow_content-cta .claim-now-btn").removeClass("btn--loading");
...
$(document).on('click', '#membership-game-lp-1yr', function (e) {
        ...
        $.getJSON('/cart.js', function(cart) {
            // Check if items are already in the cart
            var existingItem = cart.items.find(item => item.id === 44746786341061);
    
            if (existingItem) {
                // If item exists, update quantity
                alert("It looks like your cart already contains a similar item. Please remove that one first.");
                $('#membership-game-lp-1yr').removeClass('btn--loading');
            } else {
                // If item does not exist, add new items
                $.ajax({
                    url: '/cart/add',
...
window.BTGSubscriptionManager = {
  getSubscriptionRank: function (itemOrId, sellingPlanId) {
    var vId = parseInt(
      typeof itemOrId === "object" && itemOrId !== null
        ? itemOrId.variant_id || itemOrId.id
        : itemOrId,
    );
    var spId =
      sellingPlanId ||
      (itemOrId &&
      itemOrId.selling_plan_allocation &&
      itemOrId.selling_plan_allocation.selling_plan
        ? itemOrId.selling_plan_allocation.selling_plan.id
        : null);
    ...
    if (typeof itemOrId === "object" && itemOrId !== null) {
      str = (
        ...
        (itemOrId.selling_plan_allocation &&
        itemOrId.selling_plan_allocation.selling_plan
          ? itemOrId.selling_plan_allocation.selling_plan.name
          : "")
      ).toLowerCase();
    }
    if (
      str.indexOf("3 year") !== -1 ||
      str.indexOf("3-year") !== -1 ||
      str.indexOf("36 month") !== -1
    )
      return 4;
    if (
      str.indexOf("1 year") !== -1 ||
      str.indexOf("1-year") !== -1 ||
      str.indexOf("12 month") !== -1 ||
      str.indexOf("annual") !== -1
    )
      return 3;
    if (str.indexOf("6 month") !== -1 || str.indexOf("6-month") !== -1)
      return 2;
    if (
      str.indexOf("1 month") !== -1 ||
      str.indexOf("1-month") !== -1 ||
      str.indexOf("monthly") !== -1
    )
      return 1;
  ...
  isSubscriptionItem: function (item) {
    ...
    var spId =
      item.selling_plan_allocation && item.selling_plan_allocation.selling_plan
        ? parseInt(item.selling_plan_allocation.selling_plan.id)
        : null;
    return !!(
      ...
      (item.properties &&
        (item.properties.subscription_id || item.properties._is_subscription))
    );
  },
  ...
  addQRLandingSubscription: function (
    targetVariantId,
    targetSellingPlanId,
    $btn,
  ) {
    ...
    targetSellingPlanId = targetSellingPlanId
      ? parseInt(targetSellingPlanId)
      : null;
    var targetRank = self.getSubscriptionRank(
      targetVariantId,
      targetSellingPlanId,
    );

    var $allBtns = $(
      ".add_6mos_membership_qr, .add_1yr_membership_qr, .rainmakerPreOrderUpsell_lp",
    );
    if ($btn && $btn.length) {
    ...
    return fetch("/cart.js?_=" + Date.now())
      ...
      .then(function (cart) {
        var tSuffix =
          (window.theme &&
            window.theme.settings &&
            window.theme.settings.pageTemplateSuffix) ||
          "";
        ...
        var keysToRemove = lesserSubs.map(function (item) {
          return item.key;
        });
...
$(document).on("click", "#membership-game-lp-1yr", function (e) {
  ...
  $.getJSON("/cart.js", function (cart) {
    ...
    if (existingItem) {
      alert(
        "A similar item was found in your cart. We’ve removed it and added this one for you.",
      );
  ...
  $.getJSON("/cart.js?_=" + Date.now(), function (cart) {
    ...
    var existingItem = (cart.items || []).find(function (item) {
      ...
      return (
        vId === targetId ||
        vId === 42402147008709 ||
        (subMgr && subMgr.getSubscriptionRank(item) === 4)
      );
    });
    ...
    var keysToRemove = subs.map(function (item) {
      return item.key;
    });
    var cleanPromise =
      keysToRemove.length && subMgr
        ? subMgr.removeLineItems(keysToRemove)
        : Promise.resolve();
...
$(document).on("click", "#membership-game-lp-1yr-hero", function (e) {
  ...
  $.getJSON("/cart.js", function (cart) {
    ...
    if (existingItem) {
      // If item exists, update quantity
      alert("Membership product is already in the cart.");
      $("#membership-game-lp-1yr-hero").removeClass("btn--loading");
    } else {
      // If item does not exist, add new items
      $.ajax({
        url: "/cart/add",
...
$(document).on("click", ".free-gift-card-membership", function (e) {
  ...
  $.getJSON("/cart.js?_=" + Date.now(), function (cart) {
    ...
    var targetRank = subMgr
      ? subMgr.getSubscriptionRank(targetId, targetPlan)
      : 1;
    var longerSub = subs.find(function (item) {
...
$(document).on("click", ".player_plus_bundle_prime_day", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    ...
    if (existingItem) {
      // If item exists, update quantity
      alert("Membership product is already in the cart.");
      $(".player_plus_bundle_prime_day").removeClass("btn--loading");
    } else {
      // If item does not exist, add new items
      $.ajax({
        url: "/cart/add",
...
$(document).on("click", ".tips_golf_bundle_prime_day", function (e) {
  e.preventDefault();
  $(this).addClass("btn--loading");
  ...
  $.getJSON("/cart.js", function (cart) {
    $.ajax({
      ...
      data: formParams,
      success: function (data) {
        $(".tips_golf_bundle_prime_day").removeClass("btn--loading");
...
$(document).on("click", ".add_6mos_membership_qr", function (e) {
  e.preventDefault();
  window.BTGSubscriptionManager.addQRLandingSubscription(
    46056776990917,
    7328661701,
    $(this),
  );
...
$(document).on(
  "click",
  ".rainmakerPreOrderUpsell_lp, .add_1yr_membership_qr",
  function (e) {
    e.preventDefault();
    window.BTGSubscriptionManager.addQRLandingSubscription(
      45793869299909,
...
document.addEventListener("DOMContentLoaded", function () {
  const addToCartBtn = document.querySelector("#captainPro_upshell");
  if (!addToCartBtn) {
    return;
  }
...
// document.addEventListener('DOMContentLoaded', function () {
//   const addToCartBtn = document.querySelector('#playerPro_upshell');
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addToCartBtn.addEventListener("click", function (event) {
    ...
    const variantId1 = button.getAttribute("data-id");
    const variantId2 = button.getAttribute("data-id-two");
    ...
    if (variantId1) {
      fetch("/cart/add.js", {
        ...
        .then(() => {
          // After first product is added, add the second one (if exists)
          if (variantId2) {
            return fetch("/cart/add.js", {
              method: "POST",
              headers: {
        ...
        .then(() => {
          ...
          document.dispatchEvent(
            new CustomEvent("ajaxProduct:added", {
              detail: { product: {} }, // you can pass empty or first product
            }),
          );
        })
        .catch((error) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  variantInputs.forEach((input) => {
    input.addEventListener("change", function () {
      // Get the parent fieldset of the clicked input
      const fieldset = input.closest("fieldset");

      // Remove "active" class from all labels in the same fieldset
      fieldset.querySelectorAll(".variant__button-label").forEach((label) => {
        label.classList.remove("active");
      });

      // Add "active" class to the selected label
      const selectedLabel = fieldset.querySelector(`label[for="${input.id}"]`);
      if (selectedLabel) {
...
/*-------------------------
  FOR GLOBAL ADD TO CART LOGIC

  Usage: Just add this class 'cart_cta' to your 'button' or 'a' tag.
  Then add 'data-id' to your element then add your product variant ID. 
  You're good to go then.
  
  PLEASE DON'T MESS AROUND THIS CODE, THANKS!
  ----------------*/
...
//       })
//         .then((response) => {
//           if (!response.ok) throw new Error('Failed to add first product');
//           return response.json();
//         })
//         .then(() => {
//           // Add second product only if it exists
//           if (variantId2) {
//             return fetch('/cart/add.js', {
//               method: 'POST',
//               headers: {
...
//           // Trigger custom event (optional: you can pass more details if needed)
//           document.dispatchEvent(
//             new CustomEvent('ajaxProduct:added', {
//               detail: {
//                 productId: variantId1,
//                 secondProductId: variantId2 || null
//               }
//             })
//           );
//         })
  ...
  addToCartBtns.forEach((button) => {
    button.addEventListener("click", function (event) {
      ...
      if (subMgr) {
        ...
        var isSub = subMgr.isSubscriptionItem({
          variant_id: targetVId,
          selling_plan_allocation: targetPlan
            ? { selling_plan: { id: targetPlan } }
            : null,
        });
        ...
        if (isSub) {
          fetch("/cart.js?_=" + Date.now())
            ...
            .then(function (cart) {
              ...
              var targetRank = subMgr.getSubscriptionRank(
                targetVId,
                targetPlan,
              );
              var longerSub = subs.find(function (item) {
              ...
              var cleanupPromise = lesserKeys.length
                ? subMgr.removeLineItems(lesserKeys)
                : Promise.resolve();
      ...
      function proceedWithAddToCart() {
        ...
        fetch("/cart/add.js", {
          ...
          .catch((error) => {
            ...
            console.error("Cart add error:", error);
            alert(
              "There was an issue adding the product to the cart. Please try again.",
            );
          });
      }
    });
